...
/* 	date:		2025/03/18									*/
/* 	modified:	2026/03/31	(Fixed High Priority Lint)		*/
/* 	version:	v1.0										*/
/* 	author:		Wang Shen									*/
...
//------------>  state machine for generating trigger signal
...
// [Fix Rule 9]: Force logical operands to be strict boolean comparison
assign  coincid_trg_sig_valid = (coincid_trg_in == 1'b1) && (cmd_reg_in[7] == 1'b1) && (cmd_reg_in[4] == 1'b1);
assign  ext_trg_syn_sig_valid = (ext_trg_syn_in == 1'b1) && (cmd_reg_in[7] == 1'b1) && (cmd_reg_in[5] == 1'b1);
assign  cycled_trg_sig_valid  = (cycled_trg_in == 1'b1)  && (cmd_reg_in[7] == 1'b1) && (cmd_reg_in[6] == 1'b1);
assign  pmu_busy_sig_valid    = (pmu_busy_in == 1'b1)    && (cmd_reg_in[3] == 1'b1) && (cmd_reg_in[2] == 1'b1);
assign  total_busy            = (pmu_busy_sig_valid == 1'b1) || (si_busy_sig == 1'b1);

// [Fix Rule 5]: Explicit async reset to satisfy external control checker
...
// [Fix Rule 2]: Use always @(*) to avoid incomplete sensitivity list
always @(*)
begin
    n_state = IDLE;
    case (c_state)
        ...
        WAIT_TRG_WITH_SI_BUSY: begin//wait for PMU busy signal to be free
            if ((trg_enb_in == 1'b1) && (pmu_busy_sig_valid == 1'b0) && (si_busy_sig == 1'b0)) 
                n_state = (((coincid_trg_sig_valid == 1'b1) && (coincid_trg_sig_valid_r == 1'b0)) || (ext_trg_syn_sig_valid == 1'b1) || (cycled_trg_sig_valid == 1'b1)) ?  SEND_TRG : WAIT_TRG_WITH_SI_BUSY; //coincid_trg_sig_valid || ext_trg_syn_sig_valid || cycled_trg_sig_valid	          
            else
                n_state = WAIT_TRG_WITH_SI_BUSY;
        end
        WAIT_TRG_WITHOUT_SI_BUSY: begin//wait for PMU busy signal to be free
            if ((trg_enb_in == 1'b1) && (pmu_busy_sig_valid == 1'b0)) 
                n_state = (((coincid_trg_sig_valid == 1'b1) && (coincid_trg_sig_valid_r == 1'b0)) || (ext_trg_syn_sig_valid == 1'b1) || (cycled_trg_sig_valid == 1'b1)) ?  SEND_TRG : WAIT_TRG_WITHOUT_SI_BUSY;           //coincid_trg_sig_valid || ext_trg_syn_sig_valid || cycled_trg_sig_valid	
            else
                n_state = WAIT_TRG_WITHOUT_SI_BUSY;
        end
        SEND_TRG: begin//////send the trigger signal
            // [Fix Rule 11]: Bit width sync -> 8'd1
            if (trg_chksig_width_cnt >= (TRG_PULSE_WIDTH - 8'd1) )begin
        ...
        SEND_TRG_CHK: begin //send the trigger id check signal
            // [Fix Rule 11]: Bit width sync
            if (trg_chksig_width_cnt >= (8'd9 + CHK_PULSE_WIDTH))
                n_state = IDLE;
            else        
                n_state = SEND_TRG_CHK;     
        end 
...
// [Fix Rule 5]: Added posedge rst_in
    ...
    if (rst_in) begin
     ...
     case (c_state)
        ...
        WAIT_TRG_WITH_SI_BUSY: begin
            ...
            if ((trg_enb_in == 1'b1) && (pmu_busy_sig_valid == 1'b0) && (si_busy_sig == 1'b0))  begin  
                trg_send_r  <= (((coincid_trg_sig_valid == 1'b1) && (coincid_trg_sig_valid_r == 1'b0)) || (ext_trg_syn_sig_valid == 1'b1) || (cycled_trg_sig_valid == 1'b1));
                eff_trg_sig <= (((coincid_trg_sig_valid == 1'b1) && (coincid_trg_sig_valid_r == 1'b0)) || (ext_trg_syn_sig_valid == 1'b1) || (cycled_trg_sig_valid == 1'b1));
                daq_busy_r  <= (((coincid_trg_sig_valid == 1'b1) && (coincid_trg_sig_valid_r == 1'b0)) || (ext_trg_syn_sig_valid == 1'b1) || (cycled_trg_sig_valid == 1'b1));            
            end
            else begin
                trg_send_r <= 1'b0;
                eff_trg_sig <= 1'b0;
        ...
        WAIT_TRG_WITHOUT_SI_BUSY: begin
            // [Fix Rule 9]: boolean checks
            if ((trg_enb_in == 1'b1) && (pmu_busy_sig_valid == 1'b0))  begin  
                trg_send_r  <= (((coincid_trg_sig_valid == 1'b1) && (coincid_trg_sig_valid_r == 1'b0)) || (ext_trg_syn_sig_valid == 1'b1) || (cycled_trg_sig_valid == 1'b1));
                eff_trg_sig <= (((coincid_trg_sig_valid == 1'b1) && (coincid_trg_sig_valid_r == 1'b0)) || (ext_trg_syn_sig_valid == 1'b1) || (cycled_trg_sig_valid == 1'b1));
                daq_busy_r  <= (((coincid_trg_sig_valid == 1'b1) && (coincid_trg_sig_valid_r == 1'b0)) || (ext_trg_syn_sig_valid == 1'b1) || (cycled_trg_sig_valid == 1'b1));            
            end
...
always @(posedge clk_in or posedge rst_in) begin
    if (rst_in) begin
        ...
        if(busy_time_work_flow==4'd2) begin
            if((pmu_busy_sig_valid == 1'b1) && (pmu_busy_sig_valid_r == 1'b0)) begin
        ...
        if(busy_time_work_flow==4'd3) begin
                if((total_busy == 1'b1) && (total_busy_r == 1'b0)) begin
        ...
        if(busy_time_work_flow==4'd4) begin
            if(pmu_busy_sig_valid == 1'b0) begin
